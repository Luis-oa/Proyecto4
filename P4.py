...
# Moduladora I y Q para la modulación QPSK
...
portadora_modIQ = np.cos(2*np.pi*fc*t_periodo) + np.sin(2*np.pi*fc*t_periodo)

def modulador_I_Q(bits, fc, mpp):
    ...
    for i, bit in enumerate(bits):
        if bit == 1:
            senal_Tx[i*mpp : (i+1)*mpp] = portadora_modIQ
            moduladora[i*mpp : (i+1)*mpp] = 1    
        else:
            senal_Tx[i*mpp : (i+1)*mpp] = portadora_modIQ * -1
    
    # 5. Calcular la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    return senal_Tx, Pm, portadora_modIQ, moduladora


def demodulador_IQ(senal_Rx, portadora_modIQ, mpp):
    ...
    # Energía de un período de la portadora
    Es = np.sum(portadora_modIQ**2)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * portadora_modIQ
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
senal_Tx, Pm, portadora_modIQ, moduladora = modulador_I_Q(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_IQ(senal_Rx, portadora_modIQ, mpp)
...
# Visualizar el cambio entre las señales de s(t) para las distintas variables aleatorias A1 y A2
fig, (f1, f2, f3, f4) = plt.subplots(nrows=4, sharex=True, figsize=(14, 7))

#Frecuencia angular de las ondas
w_o = 2*np.pi*fc

# La onda cuadrada moduladora (bits de entrada)
p1 = np.cos(w_o*t) + np.sin(w_o*t)
f1.plot(p1, color='r', lw=2) 
f1.set_ylabel('$s1(t) - [1,1]$')

# La onda cuadrada moduladora (bits de entrada)
p2 = -1*np.cos(w_o*t) - np.sin(w_o*t)
f2.plot(p2, color='b', lw=2) 
f2.set_ylabel('$s2(t) - [-1,-1]$')

# La onda cuadrada moduladora (bits de entrada)
p3 = np.cos(w_o*t) - np.sin(w_o*t)
f3.plot(p3, color='g', lw=2) 
f3.set_ylabel('$s3(t) - [1,-1]$')

# La onda cuadrada moduladora (bits de entrada)
p4 = -1*np.cos(w_o*t) + np.sin(w_o*t)
f4.plot(p4, color='c', lw=2) 
f4.set_ylabel('$s4(t) - [-1,1]$')
f4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
print("La media de s1(t) es: ", np.mean(p1)) 
print("La media de s2(t) es: ", np.mean(p2)) 
print("La media de s3(t) es: ", np.mean(p3)) 
print("La media de s4(t) es: ", np.mean(p4))
print("\nComo se puede apreciar, la suma de las medias de cada realización es 0.")


# In[17]:


# Promedio de las realizaciones de la senal_Tx
...
# Promedio de las N realizaciones en cada instante de senal_Tx (cada punto en t)
P = [np.mean(senal_Tx) for i in range(len(t))]

#Promedio en el tiempo para las realizaciones

print("El promedio para las realizaciones del proceso para la senal_Tx es: ", round(np.mean(P),5))


# Se puede apreciar que, el promedio de las realizaciones del proceso para las senal_Tx es aproximadamente igual a la suma de las medias de cada valor de s(t) para la modulación QPSK con sus variables aleatorias (0 vs 0.00241). Se puede concluir que existe estacionaridad si se mantiene un índice de tolerancia en el promedio de los valores para la señal experimental.
...
A = 1/16
sigma = math.sqrt(A) #Varianza de las variables aleatorias
...
# Tiempo de simbolo, periodo onda portadora
Ts = 1 / fc
